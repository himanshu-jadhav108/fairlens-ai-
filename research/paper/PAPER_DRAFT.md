...
As automated decision systems are increasingly deployed in high-impact domains—including criminal justice risk assessment, credit underwriting, and demographic classification (Buolamwini & Gebru, 2018)—regulatory and policy frameworks increasingly emphasize risk management, documentation, transparency, and accountability for algorithmic systems (Raji et al., 2020). In practice, algorithmic auditing typically generates multi-dimensional quantitative evidence: demographic parity differences (DPD), equalized odds differences (EOD), disparate impact ratios (DI), performance trade-off frontiers, and Shapley additive feature attributions (SHAP). 

While quantitative auditing tools (such as Fairlearn [Weerts et al., 2023] and AIF360 [Bellamy et al., 2018]) compute these metrics accurately, non-technical stakeholders—including compliance attorneys, risk officers, and impacted individuals—face steep barriers in interpreting dense mathematical tables and multi-group disparate impact ratios. Consequently, organizations increasingly explore Large Language Models (LLMs) as generative translation layers to synthesize quantitative audit outputs into accessible, executive-facing natural language reports.
...
1. **Demographic Parity Difference (DPD):** Measures disparity in selection rates (Barocas et al., 2019):
   $$\text{DPD} = |P(\hat{Y}=1 \mid A=1) - P(\hat{Y}=1 \mid A=0)|$$
2. **Equalized Odds Difference (EOD):** Enforces parity of true positive rates and false positive rates (Hardt et al., 2016):
...
Prior theoretical and critical review work has established incompatibility results among several commonly used fairness criteria under specific assumptions (Corbett-Davies & Goel, 2018; Kleinberg et al., 2017; Chouldechova, 2017), highlighting the trade-offs involved in interpreting fairness metrics. Explaining fairness audits thus requires navigating subtle, competing definitions of parity.
...
1. Algorithmic fairness toolkits (e.g., Fairlearn [Weerts et al., 2023], AIF360 [Bellamy et al., 2018]) compute metrics and mitigation trade-offs, but do not themselves provide a standardized claim-level evaluation mechanism for validating natural-language summaries of those metrics.
...
*Methodological Scoping Note:* While these research questions formulate conceptual dimensions of explanation fidelity in fairness auditing, our empirical investigation evaluates Gemini 2.5 Flash as an exemplar foundation model within a controlled benchmark matrix.
...
2. **COMPAS Recidivism:** $N = 7,214$ instances, 53 attributes (Angwin et al., 2016). Sensitive feature: `race` (Privileged: `Caucasian`, Unprivileged: `African-American`). Target: two-year violent recidivism (`two_year_recid == 1`).
...
2. **Fairness Mitigations:**
   - *Correlation Remover (Pre-processing):* Linear projection removing sensitive correlation from feature representations while preserving geometric variance (as implemented in Fairlearn; Weerts et al., 2023).
   - *Threshold Optimizer (Post-processing):* Optimizes group-specific decision thresholds to equalize selection rates (demographic parity) subject to utility maximization (Hardt et al., 2016).
...
This formulation accepts legitimate roundings (e.g., $0.1895 \to 0.190$ or $18.9\% \to 19.0\%$) while strictly penalizing numerical distortions exceeding $1.5$ percentage points or $5\%$ relative variance. This tolerance formulation represents an a priori methodological design choice—selected prior to evaluation—intended to permit standard scientific rounding while penalizing gross numerical distortion; alternative absolute or relative thresholds would yield different absolute faithfulness percentages without altering the relative performance gap against the deterministic baseline. Figure 8 contrasts this accepted rounding formulation against authentic canonical failure cases.
...
> [!NOTE]
> Methodological Note on Directional Missingness & Significance: The preliminary statistical analysis reported prior to audit remediation indicated $p = 0.0396$ on $N = 29$. That result was artificially distorted by duplicate retry artifacts. On the true canonical 36 matrix ($N = 23$ evaluable pairs), the directional difference is non-significant. Directional faithfulness was evaluated only for conditions in which the generated explanation contained an evaluable directional assertion. Because 13 of 36 canonical conditions contained no directional assertions, this analysis is subject to potential selection effects: the observed directional-faithfulness estimate does not characterize conditions in which the model made no directional claim. While we do not speculate on the underlying generative cause for these omissions, this missingness represents an inherent methodological constraint of conditional assertion filtering.
...
|---|---|---|---|---|---|---|
| **Benchmark Dataset** | Adult Census | 12 | $92.57\% \pm 8.42\%$ | $98.98\% \pm 2.70\%$ | $5.24\% \pm 6.87\%$ | 7 / 12 |
| | COMPAS Recidivism | 12 | $87.62\% \pm 12.25\%$ | $85.00\% \pm 35.05\%$ | $8.40\% \pm 8.84\%$ | 8 / 12 |
| | German Credit | 12 | $84.21\% \pm 10.63\%$ | $99.17\% \pm 2.36\%$ | $11.89\% \pm 9.62\%$ | 8 / 12 |
| **Predictive Model** | Logistic Regression | 18 | $84.76\% \pm 11.65\%$ | $90.52\% \pm 28.63\%$ | $10.88\% \pm 9.62\%$ | 12 / 18 |
| | Random Forest | 18 | $91.51\% \pm 9.01\%$ | $98.18\% \pm 6.03\%$ | $6.14\% \pm 7.23\%$ | 11 / 18 |
| **Fairness Mitigation** | Correlation Remover | 18 | $90.15\% \pm 10.34\%$ | $91.07\% \pm 28.75\%$ | $6.32\% \pm 7.18\%$ | 12 / 18 |
| | Threshold Optimizer | 18 | $86.11\% \pm 11.20\%$ | $97.58\% \pm 6.16\%$ | $10.71\% \pm 9.74\%$ | 11 / 18 |

### Descriptive Subgroup Observations:
1. **Benchmark Datasets:** Within the evaluated benchmark, German Credit conditions exhibited lower mean numerical faithfulness ($84.21\% \pm 10.63\%$) and higher mean unsupported rates ($11.89\% \pm 9.62\%$) than Adult Census conditions ($92.57\% \pm 8.42\%$ and $5.24\% \pm 6.87\%$) and COMPAS conditions ($87.62\% \pm 12.25\%$ and $8.40\% \pm 8.84\%$). These comparisons are strictly descriptive condition-level observations within this benchmark; the study design was not powered or controlled to support causal claims or generalized inferences regarding dataset domain properties.
2. **Predictive Model Families:** Within the evaluated benchmark, Random Forest conditions exhibited higher mean numerical faithfulness ($91.51\% \pm 9.01\%$) and lower mean unsupported rates ($6.14\% \pm 7.23\%$) than Logistic Regression conditions ($84.76\% \pm 11.65\%$ and $10.88\% \pm 9.62\%$). These descriptive differences characterize the evaluated runs and do not imply an inherent model-family effect across arbitrary classification tasks.
3. **Mitigation Paradigms:** Within the evaluated benchmark, Threshold Optimizer conditions exhibited higher mean directional consistency ($97.58\% \pm 6.16\%$) alongside higher mean unsupported claim rates ($10.71\% \pm 9.74\%$) compared to Correlation Remover conditions ($91.07\% \pm 28.75\%$ and $6.32\% \pm 7.18\%$).
...
Our findings demonstrate that deploying generative models like Gemini 2.5 Flash as autonomous report generators in algorithmic auditing introduces quantifiable risks. While the evaluated model excels at synthesizing high-level narrative structure, an $11.87$ percentage point numerical degradation and an $8.51\%$ condition-level rate of unsupported assertions observed in this benchmark indicate that generative explanations cannot be treated as self-verifying audit records in compliance-oriented workflows where quantitative claims require independent verification.
...
2. **Prompting Paradigm:** Generations were elicited using prompt `combined_audit_v1` at temperature $0.2$. Alternative prompting strategies (e.g., chain-of-thought, few-shot prompting) or fine-tuning may influence faithfulness rates.
...
4. **Secondary Adjudicator Scope & Human Validation Absence:** While independent LLM adjudication was performed with Llama 3 8B to assess sensitivity, no expert human panel annotation was conducted. The high agreement rate (98/100 supported) also suggests potential positive acquiescence bias in the secondary model. The study explicitly does not claim human validation.
5. **Directional Missingness & Selection Effects:** Directional faithfulness was computed across the 23 conditions where directional assertions were generated. In 13 conditions, no directional assertions were produced. The reported directional faithfulness reflects only evaluable claims and does not characterize conditions where the model chose not to articulate directional transitions.
...
- *Measurement Error & Tolerance Design:* The evaluator's regularized tolerance window ($\pm 0.015$ / $5\%$) represents an a priori design choice; alternative tolerances would affect numerical faithfulness levels. Within the 100-claim sensitivity sample, the secondary adjudicator identified no contradictions among claims classified as supported by the deterministic evaluator, though this sensitivity analysis does not substitute for human annotation.
- *Clustered Experimental Structure:* The 36 canonical conditions share underlying datasets and model families across the 3 seeds, meaning observations exhibit clustered structure rather than independent draws. We report non-parametric Wilcoxon signed-rank tests alongside paired t-tests to provide robust corroboration.
- *Contamination Risk Remediation:* Quarantining legacy retry runs eliminated sample inflation ($N=43 \to N=36$) and corrected erroneous directional significance ($p=0.0396 \to p=0.1979$).
...
These results indicate that while Gemini 2.5 Flash capably conveyed high-level directional disparities in this benchmark, its observed numerical drift and unhedged compliance assertions highlight the importance of pairing generative explanations with deterministic verification before incorporating them into regulated algorithmic auditing workflows. Future work will extend this framework to multi-agent iterative verification architectures, evaluate few-shot mitigation prompts, and conduct formal human-subject compliance trials.
...
3. Bellamy, R. K. E., Dey, K., Hind, M., Hoffman, S. C., Houde, S., Kannan, K., Lohia, P., Martino, J., Mehta, S., Mojsilovic, A., Nagar, S., Ramamurthy, K. N., Richards, J., Saha, D., Sattigeri, P., Singh, M., Varshney, K. R., & Zhang, Y. (2018). AI Fairness 360: An extensible toolkit for detecting, understanding, and mitigating unwanted algorithmic bias. *arXiv preprint arXiv:1810.01943*.
...
10. Hardt, M., Price, E., & Srebro, N. (2016). Equality of opportunity in supervised learning. *Advances in Neural Information Processing Systems (NeurIPS)*, 3315–3323.
11. Jacovi, A., & Goldberg, Y. (2020). Towards Faithfully Interpretable NLP Systems: How Should We Define and Evaluate Faithfulness? *Association for Computational Linguistics (ACL)*, 4198–4205.
12. Kleinberg, J., Mullainathan, S., & Raghavan, M. (2017). Inherent Trade-Offs in the Fair Determination of Risk Scores. *Innovations in Theoretical Computer Science (ITCS)*.
13. Kryscinski, W., McCann, B., Xiong, C., & Socher, R. (2020). Evaluating the Factual Consistency of Abstractive Text Summarization. *Empirical Methods in Natural Language Processing (EMNLP)*, 9332–9346.
14. Lakkaraju, H., Arsov, N., & Bastani, O. (2020). Robust and Stable Black Box Explanations. *International Conference on Machine Learning (ICML)*, PMLR 119, 5628–5638.
...
16. Min, S., Krishna, K., Lyu, X., Lewis, M., Yih, W. T., Koh, P. W., Iyyer, M., Zettlemoyer, L., & Hajishirzi, H. (2023). FActScore: Fine-grained Atomic Evaluation of Factual Precision in Long Form Text Generation. *Empirical Methods in Natural Language Processing (EMNLP)*, 12076–12100.
17. Raji, I. D., Smart, A., White, R. N., Mitchell, M., Gebru, T., Hutchinson, B., Smith-Loud, J., Theron, D., & Barnes, P. (2020). Closing the AI Accountability Gap: Defining an End-to-End Framework for Internal Algorithmic Auditing. *ACM Conference on Fairness, Accountability, and Transparency (FAccT)*, 33–44.
18. Ribeiro, M. T., Singh, S., & Guestrin, C. (2016). "Why Should I Trust You?": Explaining the Predictions of Any Classifier. *ACM SIGKDD International Conference on Knowledge Discovery and Data Mining (KDD)*, 1135–1144.
19. Slack, D., Hilgard, S., Jia, E., Singh, S., & Lakkaraju, H. (2020). Fooling LIME and SHAP: Adversarial Attacks on Post Hoc Explanation Methods. *AAAI/ACM Conference on AI, Ethics, and Society (AIES)*, 180–186.
20. Turpin, M., Michael, J., Perez, E., & Bowman, S. R. (2023). Language Models Don't Always Say What They Think: Unfaithful Explanations in Chain-of-Thought Prompting. *Advances in Neural Information Processing Systems (NeurIPS)*.
21. Weerts, H., Dudík, M., Edgar, R., Jalali, A., Lutz, R., & Madaio, M. (2023). Fairlearn: Assessing and improving fairness of AI systems. *Journal of Machine Learning Research (JMLR)*, 24(257), 1–8.
22. Wiegreffe, S., & Pinter, Y. (2019). Attention is not not Explanation. *Empirical Methods in Natural Language Processing (EMNLP)*, 11–20.
