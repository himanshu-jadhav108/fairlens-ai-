...
TOP_K_PATTERNS = [
    r"([a-zA-Z0-9_]+)\s+(?:was|is|ranked)\s+among\s+the\s+top\s+(?:features|predictors|variables)",
    r"([a-zA-Z0-9_]+)\s+(?:was|is)\s+one\s+of\s+the\s+top\s+(?:features|predictors|variables)",
    r"top\s+(?:features|predictors)\s+included?\s+([a-zA-Z0-9_]+)"
]
            ...
            for pattern in MOST_IMPORTANT_PATTERNS:
                ...
                if m:
                    ...
                    if matched_gt_name is None:
                        # Hallucinated feature not in dataset
